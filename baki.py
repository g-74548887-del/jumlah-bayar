...
if st.button("Kira Baki"):
    ...
    if baki < 0:
        ...
        st.markdown("""
        <div class='resit'>
        <h3>🧾 Resit Mini Transaksi</h3>
        """, unsafe_allow_html=True)

        st.markdown(f"**Jumlah Harga:** RM {jumlah_harga:.2f}")
        st.markdown(f"**Duit Diberi:** RM {duit_diberi:.2f}")
        st.markdown(f"**Baki:** RM {baki:.2f}")

        if peranan == "👩‍💼 Peniaga":
            st.markdown("_Baki perlu diberi kepada pembeli._")
        else:
            st.markdown("_Baki yang anda akan terima._")
                    ...
                    if os.path.exists(img_path):
                        ...
                        st.markdown(
                            ...
                            <div class='row-duit'>
                                ...
                                <span style='font-size:18px;font-weight:bold;margin-left:5px;'>RM{nilai:.2f}</span>
                            </div>
                            """,
                            unsafe_allow_html=True
                        )
